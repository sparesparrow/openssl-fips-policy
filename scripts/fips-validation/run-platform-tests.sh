...
if openssl fipsinstall -verify -module "$FIPS_MODULE_PATH" | grep -q "INSTALL_SELF_TEST_KATS_RUN"; then
    log_success "FIPS self-tests passed"
else
    log_error "FIPS self-tests failed"
    exit 1
fi
...
# Cleanup
rm -f testfile.txt "fipsmodule-${PLATFORM}-isolated.cnf"